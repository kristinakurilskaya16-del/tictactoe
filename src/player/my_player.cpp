...
Point MyPlayer::make_move(const State& state)
{
    ...
    for (const auto& c : candidates) {
        ...
        if (would_win_fast(tmp, c.x, c.y, opponent)) 
            return c;
    }
...
// мы сканируем всё поле и ищем последовательности, потом даем оценку по pattern плюс бонус за центр

// int MyPlayer::evaluate_position(const State& state, Sign player) const
// {
//     Sign opponent = (player == Sign::X) ? Sign::O : Sign::X;
...
//         for (int x = 0; x < cols; ++x)
//         {
//             Sign s = safe_get(state, x, y);

//             if (s == Sign::NONE || s == Sign::WALL)
//                 continue;
...
//                 if (safe_get(state, px, py) == s)
//                     continue;

//                 Pattern pat =
//                     SequencesAnalyzer::scan_existing_line(
//                         state, x, y, d[0], d[1], 5
//                     );

//                 int v = pat.get_pattern_weight();
...
std::vector<Point> MyPlayer::order_candidate_moves(const State& state,
    ...
    int sort_count = std::min(max_moves, (int)scored.size());

    std::partial_sort(scored.begin(), scored.begin() + sort_count, 
                      scored.end(),
...
int MyPlayer::negamax(const State& state, int depth, int alpha,
    ...
    // ThreatInfo info =
    //     ThreatEngine::analyze(state, current_player, 5);

    // ThreatInfo opp_info =
    //     ThreatEngine::analyze(state, opponent, 5);

    // if (!info.winning_moves.empty())
    //     return 5'000'000 + depth;

    // if (!opp_info.winning_moves.empty())
    //     return -5'000'000 - depth;
